...
class ConvolutionalLayer(Layer):
    def __init__(self, inputShape123, kernelSize, numKernels):
        inputDepth, inputHeight, inputWidth = inputShape123
        #self.inputShape = inputShape
        #self.numInputs = numInputs
        self.depth = numKernels
        self.inputDepth = inputDepth
        self.outputShape123 = (numKernels, inputHeight-kernelSize+1, inputWidth-kernelSize+1)
        self.kernelShape = (numKernels, inputDepth, kernelSize,kernelSize)
        self.kernels = np.random.randn(*self.kernelShape)
        #self.bias = np.random.randn(*self.outputShape123)
    def forward_prop(self, input):
        #print("convolving, input shape: ", input.shape)
        self.numInputs = input.shape[0]
        shape = self.outputShape123
        self.input = input
        self.output = np.zeros((self.numInputs, shape[0], shape[1], shape[2]))
        for num in range(self.numInputs):
            for i in range(self.depth):
                for j in range(self.inputDepth):
                    ...
                    self.output[num,i]+=signal.correlate2d(self.input[num,j], self.kernels[i,j],"valid")
...
#                     cv2.imwrite('testOut%d.jpg' %(num), test1)
    ...
    def back_prop(self,output_error,alpha):
        ...
        for num in range(self.numInputs):
            for i in range(self.depth):
                for j in range(self.inputDepth):
                    dKdE[i,j] += signal.correlate2d(self.input[num,j],output_error[num,i],"valid")
                    dIdE[num,j]+= signal.convolve2d(output_error[num,i],self.kernels[i,j], "full")
        self.kernels -= alpha*dKdE/self.numInputs
...
#for digits
net.add(ConvolutionalLayer((1,28,28), 5, 6))
net.add(PoolingLayer(2))
net.add(ConvolutionalLayer((6,12,12), 3, 12))
...
net.sgd(X_train[0:1000], Y_train[0:1000], epochs=11, alpha=0.01)
